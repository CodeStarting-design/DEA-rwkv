...
from option_train import opt,writer_loss,writer_metric
from data.data_loader import REVIDE_Dataset
...
def train(net, loader_train, loader_test, optim, criterion,scaler):
    losses = []
    ...
    for step in range(start_step + 1, steps + 1):
        ...
        writer_loss.add_scalar('L1', loss_L1.item(), step)
        writer_loss.add_scalar('CR', loss_CR.item(), step)
        writer_loss.add_scalar('total', loss.item(), step)
        ...
        if step % len(loader_train) == 0:
            ...
            for key in loss_log.keys():
                loss_key_mean=np.average(np.array(loss_log_tmp[key]))
                loss_log[key].append(loss_key_mean)
                writer_loss.add_scalar("loss_epoch"+key, loss_key_mean, step)
                loss_log_tmp[key] = []
        ...
        if (step % opt.iters_per_epoch == 0 and step <= opt.finer_eval_step) or (step > opt.finer_eval_step and (step - opt.finer_eval_step) % (5 * len(loader_train)) == 0):
            ...
            with open(os.path.join(opt.saved_data_dir, 'log.txt'), 'a') as f:
                f.write(log + '\n')
            writer_metric.add_scalar("SSIM", ssim_eval, epoch)
            writer_metric.add_scalar("PSNR", psnr_eval, epoch)
...
if __name__ == "__main__":
    ...
    train_dir = '../dataset/REVIDE_inside/'
    train_set = REVIDE_Dataset(train_dir,"Train","train",384,384,0,True)
    test_dir = '../dataset/REVIDE_inside/'
    test_set = REVIDE_Dataset(test_dir,"Test","test",256,256,0,True)
    loader_train = DataLoader(dataset=train_set, batch_size=4, shuffle=True, num_workers=4)
    loader_test = DataLoader(dataset=test_set, batch_size=1, shuffle=False, num_workers=2)
    ...
    train(net, loader_train, loader_test, optimizer, criterion,scaler)