...
wkv6_cuda = load(name="wkv6",
                 sources=["model/cuda_v6/wkv6_op.cpp", 
                          "model/cuda_v6/wkv6_cuda.cu"],
    ...
    def backward(ctx, gy):
        with torch.no_grad():
            ...
            gr = torch.empty((B, T, C), device=gy.device, requires_grad=False, 
                             dtype=torch.float32, memory_format=torch.contiguous_format)#.uniform_(-100, 100)
...
class Block(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, layer_id, shift_mode='q_shift_multihead',
                 ...
                 init_values=1e-5, post_norm=True, key_norm=False, with_cls_token=False,
                 with_cp=False):
        super().__init__()
        self.layer_id = layer_id
