...
parser.add_argument('--model_name', type=str, default='DEA-Net-CR')
...
parser.add_argument('--dataset', type=str, default='REVIDE_INSIDE')
...
if not os.path.exists(model_dir):
    ...
    opt.saved_board_dir = os.path.join(model_dir,"saved_board_loss")
    opt.saved_board_metric = os.path.join(model_dir,"saved_board_metric")
    os.mkdir(opt.saved_model_dir)
    os.mkdir(opt.saved_data_dir)
    os.mkdir(opt.saved_plot_dir)
    os.mkdir(opt.saved_infer_dir)
    os.mkdir(opt.saved_board_dir)
    os.mkdir(opt.saved_board_metric)
else:
    print(f'{model_dir} has already existed!')
    exit()

writer_loss = SummaryWriter(log_dir=os.path.join(model_dir,"saved_board_loss"))
writer_metric = SummaryWriter(log_dir=os.path.join(model_dir,"saved_board_metric"))
